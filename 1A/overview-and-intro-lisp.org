...
* Concept 2: The Structure of Lisp
  ...
  3.  **Means of Abstraction:** How do we name things to use them as elements later? (`defun`, `defvar`).

** 1. Primitives & Combinations
   Lisp uses **Prefix Notation** (operator first). This eliminates ambiguity and the need for operator precedence rules (like PEMDAS).
...
** 2. Abstraction (Naming)
   In Elisp, we define functions using ~defun~ (Define Function) and variables using ~defvar~ or ~setq~. This allows us to suppress detail. Once we define ~square~, we don't need to know *how* it works, only *what* it does.
...
* The Main Example: Newton's Method for Square Roots
  ...
  This is the highlight of the lecture. It demonstrates how to translate the Imperative "Heron's Method" directly into code.

  *Key Difficulty:* Note that ~try-guess~ calls ~improve~, and ~try-guess~ also calls *itself* (Recursion). This creates a loop without using `for` or `while`.
